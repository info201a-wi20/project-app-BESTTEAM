...
getStock <- function(f, t, country_code) {
  ...
  c = content(res, "text")
  c = fromJSON(c)
  t <- as.Date(as.POSIXct(c$chart$result[[1]]$timestamp, origin="1970-01-01"))
  l <- c$chart$result[[1]]$indicators$quote[[1]]$low[[1]]
  v <- c$chart$result[[1]]$indicators$quote[[1]]$volume[[1]]
  cl <- c$chart$result[[1]]$indicators$quote[[1]]$close[[1]]
  o <- c$chart$result[[1]]$indicators$quote[[1]]$open[[1]]
  h <- c$chart$result[[1]]$indicators$quote[[1]]$high[[1]]
